...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_folder", type=str, default="data/samples", help="待检测数据集路径")
    parser.add_argument("--batch_size", type=int, default=8, help="批大小")
    parser.add_argument("--model_def", type=str, default="  ... .cfg", help="模型配置文件路径")
    parser.add_argument("--weights_path", type=str, default="... .weights", help="模型权重路径")
    parser.add_argument("--class_path", type=str, default="... classes.names", help="类别标签路径")
    ...
    parser.add_argument("--img_size", type=int, default=416, help="图像大小")
    parser.add_argument("--checkpoint_model", type=str, help="path to checkpoint model")
    # 输入：... --checkpoint_model 参数1 --image_folder 参数2 ...

    # opt = parser.parse_args()
    opt = parser.parse_known_args()[0]
    # 上面两句作用相同，但似乎在colab/jupyter上用上面那句会出错
    ...
    os.makedirs("output", exist_ok=True)  # 创建多级目录,若已存在应该是不会覆盖的
    ...
    dataloader = DataLoader(
        ...
        shuffle=False,
        num_workers=opt.n_cpu,# 用GPU就改成0，要用cpu就改成opt.n_cpu
    )
    ...
    colors = [cmap(i) for i in np.linspace(0, 1, 20)]  # np.linspace：0到1之间20个数的等差数列
